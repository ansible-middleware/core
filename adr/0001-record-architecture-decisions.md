...
`Ansible Middleware` has several objectives that make the practice and
...
We will document every architecture-level decision for `Ansible Middleware` with an
...
1. Developers must write an ADR and submit it for review before
   ...
   decision that affects the way Ansible Middleware project is affected.
